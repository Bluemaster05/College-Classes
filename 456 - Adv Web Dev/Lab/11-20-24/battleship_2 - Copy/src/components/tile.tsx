    ...
    function whenMouseLeave() {
        if (props.interaction) {
            if (props.numturns < 2) {
                ...
                props.setGb(prevGb => {
                    let thisType = getTile(prevGb, props.curPlayer, 'attack', { y: +props.id!.slice(0, 1), x: +props.id.slice(1) }).type
    ...
    function onMouseClick() {
        if (props.interaction) {
            if (props.numturns < 2) {
                props.setGb((prevGb) => {
                    try {
                        let tiles = getPositionsForShip(prevGb, props.curPlayer, props.selectedShip!, { y: +props.id!.slice(0, 1), x: +props.id.slice(1) }, props.oreintation)
                        let placedShip = placeShip(prevGb, props.curPlayer, props.selectedShip!, tiles)
                ...
                props.setGb((prevGb) => {
                    if (!props.hasClicked) {
                        if (getTile(prevGb, props.curPlayer, 'attack', { y: +props.id!.slice(0, 1), x: +props.id.slice(1) }).type === TileType.TARGETING) {
                            props.setHasClicked(true)
                            setCursor('default')
                            return attemptToHitShip(prevGb, getOtherPlayer(props.curPlayer), { y: +props.id!.slice(0, 1), x: +props.id.slice(1) })
                        } else {
                            return prevGb
                        }
                    } else {
